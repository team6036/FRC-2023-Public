package com.peninsula.frc2023.vision;
 ...
 * at the bottom-right corner of the blue alliance wall. {@link
 * #setOrigin(AprilTagFieldLayout.OriginPosition)} can be used to change the poses returned from
 * {@link AprilTagFieldLayout#getTagPose(int)} to be from the perspective of a specific alliance.
 */
...
public class AprilTagFieldLayout {
  ...
  private final Map<Integer, List<Translation3d>> m_apriltags_corners = new HashMap<>();
  private final int[] dx = {-1, 1, 1, -1}, dy = {1, 1, -1, -1};
  ...
  public AprilTagFieldLayout(Path path) throws IOException {
    ...
    List<AprilTag> tags = getTags();

    for (AprilTag tag : tags) {
      List<Translation3d> clockwiseCorners = new ArrayList<>();
      for (int i = 0; i < 4; i++) {
        clockwiseCorners.add(
            tag.pose
                .plus(
                    new Transform3d(
                        new Translation3d(0, dx[i] * 0.15 / 2, dy[i] * 0.15 / 2), new Rotation3d()))
                .getTranslation());
      }
      m_apriltags_corners.put(tag.ID, clockwiseCorners);
    }

    m_fieldDimensions = layout.m_fieldDimensions;
    setOrigin(AprilTagFieldLayout.OriginPosition.kBlueAllianceWallRightSide);
  }
   ...
   */
  @JsonIgnore
  public void setOrigin(AprilTagFieldLayout.OriginPosition origin) {
    switch (origin) {
  ...
  /**
   * Gets an AprilTag corners by its ID.
   *
   * @param ID The ID of the tag.
   * @return The corners corresponding to that ID
   */
  @SuppressWarnings("ParameterName")
  public Optional<List<Translation3d>> getTagCorners(int ID) {
    List<Translation3d> tag = m_apriltags_corners.get(ID);
    if (tag == null) {
      return Optional.empty();
    }
